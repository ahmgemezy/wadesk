...
**Created**: 2026-04-02
**Status**: Draft
**Input**: User description: "WhatsApp Embedded Signup — Admin connects their WhatsApp Business Account (WABA) to WABDesk in under 5 minutes using Meta's Embedded Signup flow. They authenticate with Facebook, select or create a WABA, choose a phone number, and the channel is immediately active in their inbox. This is the critical onboarding step — without it no messages can be received."

## User Scenarios & Testing *(mandatory)*

### User Story 1 — Admin connects a WhatsApp number for the first time (Priority: P1)

A new WABDesk admin has just created their account. They land on the onboarding page and see a
...
permissions. Within seconds, the connected number appears as an active channel in their WABDesk inbox —
...
1. **Given** an admin is on the WABDesk dashboard with no channels connected, **When** they click
...
2. **Given** the admin has authenticated with Facebook, **When** they select an existing WABA and
   phone number and grant permissions, **Then** the channel is created in WABDesk and appears in
...
4. **Given** the channel is connected, **When** a customer sends a WhatsApp message to the
   connected number, **Then** it appears in the WABDesk inbox Unassigned queue within 3 seconds.
...
An admin needs to disconnect a WhatsApp number from WABDesk — for example, because they are
...
- What happens if the phone number selected is already registered on another WABDesk tenant?
  Blocked — Meta will surface an error during the flow. WABDesk shows a clear message: "This number is already in use."
...
- **FR-001**: Admins MUST be able to initiate the WhatsApp channel connection flow from the WABDesk dashboard without leaving the platform (popup or embedded flow).
- **FR-002**: The connection flow MUST use Meta's official Embedded Signup to authenticate with Facebook and authorize WABDesk to send/receive messages on behalf of the WABA.
...
- Meta's Embedded Signup JavaScript SDK is embedded in the WABDesk frontend; the OAuth flow opens in a popup.
- WABDesk registers a single Meta App that all tenants connect through (shared app, each tenant's WABA is independently authorized).
...
- Each connected phone number maps to exactly one WABDesk channel document per tenant.
- Webhook registration (subscribing the WABDesk webhook URL to the WABA) is handled automatically by WABDesk after the admin completes the flow — the admin never sees or touches webhook URLs.
- Phone number display names come from the Meta WABA profile; admins can override with a WABDesk-internal label.
